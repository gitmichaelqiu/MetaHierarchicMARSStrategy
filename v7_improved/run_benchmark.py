"""
V7 Improved MoA Trading Framework — Benchmark Runner
...
def plot_results(ticker, result, save_dir='../Plots'):
    ...
    fig.suptitle(f'V7 Improved MoA Strategy: {ticker}', fontsize=16, fontweight='bold')
    
    ax = axes[0]
    ax.plot(result.equity_curve.index, result.equity_curve.values,
            label=f'MoA V7 Improved ({result.total_return:.1%})', linewidth=2, color='#2196F3')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'v7_improved_{ticker}_performance.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_summary(all_results, save_dir='../Plots'):
    ...
    fig.suptitle('V7 Improved MoA Framework — Cross-Asset Benchmark', fontsize=16, fontweight='bold')
    
    x = np.arange(len(tickers)); w = 0.35
    ax = axes[0]
    ax.bar(x-w/2, [all_results[t].total_return*100 for t in tickers], w, label='MoA V7 Improved', color='#2196F3')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'v7_improved_benchmark_summary.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def run_full_benchmark(tickers=None, period='2y', debug=True):
    ...
    print(f"\n{'='*60}")
    print(f"MoA V7 Improved — Aggressive Parameter Tuning")
    print(f"  Higher Growth Baseline + Stronger Sector Boost + Relaxed Correlation")
    print(f"{'='*60}")
    ...
    if all_results:
        print(f"\n{'='*60}")
        print(f"V7 IMPROVED BENCHMARK SUMMARY")
        print(f"{'='*60}")
