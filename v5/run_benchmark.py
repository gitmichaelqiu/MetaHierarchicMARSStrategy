"""
V5 MoA Trading Framework — Benchmark Runner
Regime Momentum + VIX Fear Gauge + Drawdown-Adaptive Baseline.
...
from v5_agents.exponential_momentum_agent import ExponentialMomentumAgent


def fetch_vix_data(period: str = '2y') -> pd.Series:
    """Fetch VIX historical data aligned to trading dates."""
    try:
...
class MoAStrategyV5:
    """
    V5 MoA: Adaptive Baseline + Regime Momentum + VIX + Drawdown Control.
    """
    
    def __init__(self, vix_data: Optional[pd.Series] = None, debug: bool = True):
    ...
    def _get_vix_value(self, date) -> Optional[float]:
        """Get VIX value for the given date."""
        if self.vix_data is None or self.vix_data.empty:
            return None
        
        # Try exact date match first, then nearest prior date
        try:
            # Normalize timezone
            if hasattr(date, 'tz') and date.tz is not None:
                date = date.tz_localize(None) if date.tz is None else date.tz_convert(None)
            
            vix_idx = self.vix_data.index
            if hasattr(vix_idx, 'tz') and vix_idx.tz is not None:
                vix_idx = vix_idx.tz_convert(None)
            
            # Use date only (no time) for matching
            date_only = pd.Timestamp(date).normalize()
            
            # Find nearest available date
            diffs = abs(vix_idx.normalize() - date_only)
            nearest_idx = diffs.argmin()
            if diffs[nearest_idx] <= pd.Timedelta(days=3):
                return float(self.vix_data.iloc[nearest_idx])
        except Exception:
            pass
    ...
    def generate_signal(self, df: pd.DataFrame, current_idx: int) -> Tuple[float, str]:
        ...
        vix_value = self._get_vix_value(current_date)
        ...
        controller_output = self.controller.compute_position(
            ...
            current_vol, regime_probs, vix_value=vix_value
        )
        ...
        if self.debug and current_idx % 50 == 0:
            ...
            mom = controller_output.metadata.get('regime_momentum', {})
...
def plot_results(ticker: str, result: BacktestResult, save_dir: str = '../Plots'):
    ...
    fig.suptitle(f'V5 MoA Strategy: {ticker}', fontsize=16, fontweight='bold')
    
    ax = axes[0]
    ax.plot(result.equity_curve.index, result.equity_curve.values, 
            label=f'MoA V5 ({result.total_return:.1%})', linewidth=2, color='#2196F3')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'v5_{ticker}_performance.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_summary(all_results: Dict[str, BacktestResult], save_dir: str = '../Plots'):
    ...
    fig.suptitle('V5 MoA Framework — Benchmark Summary', fontsize=16, fontweight='bold')
    
    x = np.arange(len(tickers))
    width = 0.35
    ax = axes[0]
    ax.bar(x - width/2, [all_results[t].total_return*100 for t in tickers], width, label='MoA V5', color='#2196F3')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'v5_benchmark_summary.png'), dpi=150, bbox_inches='tight')
    plt.close()


def run_single_backtest(ticker, period, vix_data, debug=True):
    print(f"\n{'='*60}")
    print(f"Running V5 backtest for {ticker}")
    print(f"{'='*60}")
    ...
    strategy = MoAStrategyV5(vix_data=vix_data, debug=debug)
...
def run_full_benchmark(tickers=None, period='2y', debug=True):
    ...
    print(f"\n{'='*60}")
    print(f"MoA V5 Trading Framework — Full Benchmark")
    print(f"  Regime Momentum + VIX + Drawdown-Adaptive Baseline")
    print(f"{'='*60}")
    ...
    if not vix_data.empty:
        ...
        print("  WARNING: VIX data unavailable, proceeding without it")
    
    all_results = {}
    for ticker in tickers:
        result = run_single_backtest(ticker, period, vix_data, debug)
        if result:
            all_results[ticker] = result
    
    if all_results:
        print(f"\n{'='*60}")
        print(f"V5 BENCHMARK SUMMARY")
        print(f"{'='*60}")
